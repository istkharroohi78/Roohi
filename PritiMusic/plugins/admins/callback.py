...
@languageCB
async def settings_back_helper(client, CallbackQuery, _):  # FIXED: Added '_' parameter here
    await CallbackQuery.answer()
    img = random.choice(config.START_IMG_URL) if isinstance(config.START_IMG_URL, list) else config.START_IMG_URL
    await CallbackQuery.edit_message_media(
        media=InputMediaPhoto(
            media=img, 
            caption=_["start_2"].format(CallbackQuery.from_user.mention, app.mention) # FIXED: Changed '["start_2"]' to '_["start_2"]'
        ),
