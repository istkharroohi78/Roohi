...
class Call(PyTgCalls):
    ...
    async def _safe_change_stream(self, client, chat_id, file_path, video=False, extra_args=""):
        ...
        try: 
            stream = MediaStream(
                file_path, audio_parameters=AudioQuality.HIGH, 
                video_parameters=VideoQuality.HD_720p, ffmpeg_parameters=extra_args
            )
            await client.play(chat_id, stream)
            ...
            stream = MediaStream(
                file_path, audio_parameters=AudioQuality.HIGH, 
                video_parameters=VideoQuality.SD_480p, ffmpeg_parameters=extra_args
            )
            await client.play(chat_id, stream)
    ...
    async def _safe_join_call(self, assistant_to_join, chat_id, file_path, video=False):
        ...
        try: 
            stream = MediaStream(
                file_path, audio_parameters=AudioQuality.HIGH, video_parameters=VideoQuality.HD_720p
            )
            await assistant_to_join.play(chat_id, stream)
        except Exception as e:
            LOGGER(__name__).warning(f"720p Join Call failed, auto-switching to 480p: {e}")
            stream = MediaStream(
                file_path, audio_parameters=AudioQuality.HIGH, video_parameters=VideoQuality.SD_480p
            )
            await assistant_to_join.play(chat_id, stream)
    ...
    async def get_active_clients(self, chat_id):
        ...
        if chat_id in self.active_clients:
            ...
            if isinstance(val, list): clients.extend(val)
            else: clients.append(val)
        if not clients:
            try:
        ...
        for assistant in active_assistants:
            if assistant:
                try: 
                    ...
                    if not any(ign in error_msg for ign in ignore_list):
        ...
        if userbot:
            if FORCE_JOIN_LINKS:
                for link_join in FORCE_JOIN_LINKS:
                    try: await userbot.join_chat(link_join); await asyncio.sleep(0.5) 
                    except: pass
    ...
    # 🔥 10 MINUTE AUTO-LEAVE BACKGROUND TASK 🔥
    async def auto_end_task(self):
        empty_vcs = {}
        while True:
            await asyncio.sleep(60) # Har 1 minute check karega
            try:
                active_chats = list(self.active_clients.keys())
                for chat_id in active_chats:
                    assistants = await self.get_active_clients(chat_id)
                    if not assistants: continue
                    assistant = assistants[0]
                    try:
                        participants = await assistant.get_participants(chat_id)
                        if len(participants) <= 1:
                            if chat_id not in empty_vcs:
                                empty_vcs[chat_id] = datetime.now()
                            elif (datetime.now() - empty_vcs[chat_id]).total_seconds() >= 600: # 10 Minutes passed
                                try:
                                    await self.stop_stream(chat_id)
                                    LOGGER(__name__).info(f"bot ne {chat_id} ka VC chhod diya (10 Min Inactivity).")
                                except: pass
                                empty_vcs.pop(chat_id, None)
                        else:
                            if chat_id in empty_vcs:
                                empty_vcs.pop(chat_id, None)
                    except Exception:
                        pass
            except Exception:
            ...
            if logger_id:
                try:
                    ...
                    if chat_obj.username:
                        chat_url = f"https://t.me/{chat_obj.username}"
                    else:
                        chat_url = f"https://t.me/c/{str(original_chat_id).replace('-100', '')}/1"
                    ...
                    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("💬 CHAT LINK", url=chat_url)]])
                    await app.send_message(int(logger_id), log_text, reply_markup=reply_markup)
                except Exception as e:
                    LOGGER(__name__).warning(f"Failed to send Play Log to GC: {e}")
    ...
    async def start(self):
        ...
        # 🔥 STARTING THE AUTO LEAVE TASK BACKGROUND PROCESS 🔥
